...
public class RevAttributes : MonoBehaviour {
    private ServerListener mainServer;
    public AttributeLinker attributeLinkerRef;
    [System.Serializable]
    ...
    void Awake()
    {
        ...
        mainServer = FindObjectOfType<ServerListener>();
        ...
        if (attributeLinkerRef != null && attributeLinkerRef.dataType == AttributeLinker.DATA_TYPE.IFC) {
        ...
        } else if (attributeLinkerRef != null && attributeLinkerRef.dataType == AttributeLinker.DATA_TYPE.FBX) {
            string[] splitName = this.transform.name.Split(' ');
            string ID = splitName[splitName.Length - 1];
            //Debug.Log("Found ID:" + ID);
            ID = ID.Replace("[", "").Replace("]", "");
            //Debug.Log("Parsed to int:" + int.Parse(ID));
            ID_TAG = ID;
            if (UNIQUE_ID.Length > 1) {
                ID_TAG = UNIQUE_ID;
            }
